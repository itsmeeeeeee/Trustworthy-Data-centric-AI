...
def calculate_sari_or_rouge(metric: str, df: pd.DataFrame, inputs_references: dict, output_columns: str = 'output'):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action='store_true',  # Store True when f
    ...
    if TASK == 'Simplification':
        ...
        df_simp = pd.read_csv('data/100_sample/simplification_texts.csv')
        inputs = df_simp['displayed_text']
        references = df_simp['gold_reference']
        with open('data/outputs/models_output/output_simplification_all_new.json', 'r') as f:
            model_outputs = json.load(f)
    elif TASK == 'Summarisation':
        METRIC = 'rouge'
        df_summ = pd.read_csv('data/100_sample/summarisation_texts.csv')
        inputs = df_summ['displayed_text']
        references = df_summ['gold_reference']
    ...
    # Speichere by_model in eine CSV-Datei
    by_model_df = pd.DataFrame.from_dict(metrics_['by_model'], orient='index', columns=['Score'])
    by_model_df.index.name = 'Model'
    by_model_df.reset_index(inplace=True)
    by_model_df.to_csv("by_model.csv", index=False)
    print("Ergebnisse aus 'by_model' wurden in 'by_model.csv' gespeichert.")
        # Speichere by_prompt in eine CSV-Datei
    by_prompt_df = pd.DataFrame.from_dict(metrics_['by_prompt'], orient='index', columns=['Score'])
    by_prompt_df.index.name = 'Prompt'
    by_prompt_df.reset_index(inplace=True)
    by_prompt_df.to_csv("by_prompt.csv", index=False)
    print("Ergebnisse aus 'by_prompt' wurden in 'by_prompt.csv' gespeichert.")

    # Speichere by_temperature in eine CSV-Datei
    by_temperature_df = pd.DataFrame.from_dict(metrics_['by_temperature'], orient='index', columns=['Score'])
    by_temperature_df.index.name = 'Temperature'
    by_temperature_df.reset_index(inplace=True)
    by_temperature_df.to_csv("by_temperature.csv", index=False)
    print("Ergebnisse aus 'by_temperature' wurden in 'by_temperature.csv' gespeichert.")
    # Speichere by_model_prompt_temperature in eine CSV-Datei
    rows = []
    for (model, prompt, temperature), score in metrics_['by_model_prompt_temperature'].items():
        rows.append({'Model': model, 'Prompt': prompt, 'Temperature': temperature, 'Score': score})
    by_model_prompt_temperature_df = pd.DataFrame(rows)
    by_model_prompt_temperature_df.to_csv("by_model_prompt_temperature_{TASK}_summarisation.csv", index=False)
    print("Ergebnisse aus 'by_model_prompt_temperature' wurden in 'by_model_prompt_temperature.csv' gespeichert.")
    # Speichere by_model_prompt_temperature in eine CSV-Datei
    rows = []
    for (model, prompt, temperature), score in metrics_['by_model_prompt_temperature'].items():
        rows.append({'Model': model, 'Prompt': prompt, 'Temperature': temperature, 'Score': score})
    by_model_prompt_temperature_df = pd.DataFrame(rows)
    by_model_prompt_temperature_df.to_csv("by_model_prompt_temperature.csv", index=False)
    print("Ergebnisse aus 'by_model_prompt_temperature' wurden in 'by_model_prompt_temperature.csv' gespeichert.")




    # Speichern von printable_df_ in einer CSV-Datei
    ...
    if GENERATE_PLOT:
        ...
        plt.gcf().set_size_inches(4 * 1, 4 * 3)
        plt.savefig("output_heatmap.png")  # Speichere den Plot
        plt.show()  
